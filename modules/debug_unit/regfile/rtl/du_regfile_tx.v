...
//------------------------------------------ OUTPUTS ---------------------------------------------//
  ...
  output wire [NB_REG_ADDR                             - 1 : 0] o_regfile_raddr                   ,
//------------------------------------------- INPUTS ---------------------------------------------//
  input  wire                                                   i_start                           ,
  input  wire [NB_PC                                   - 1 : 0] i_pc                              ,  // PC input
  input  wire [NB_REG                                  - 1 : 0] i_regfile_data                    ,  // CPU register file input
...
//---------------------------------------- local params ------------------------------------------//
  ...
  localparam                                                    READ_LATENCY  = 3'd4              ;  // cycles to wait for regfile read
  localparam                                                    BYTE_LAST     = 3'd4              ;  // counter value after last byte
  localparam                                                    N_REGS        = 32                ;  // number of registers
...
//------------------------------------------ Registers -------------------------------------------//
  ...
  reg  [NB_REG_ADDR                                    - 1 : 0] regfile_addr_reg                  ;
  reg  [NB_REG_ADDR                                    - 1 : 0] regfile_addr_next                 ;
...
//--------------------------------------- Sequential logic ---------------------------------------//
  ...
  begin
    ...
    begin
      ...
      regfile_addr_reg <= {NB_REG_ADDR{1'b0}}                                                     ;
  ...
  begin
    ...
    case (state_reg)
      ...
      begin
        if (counter_reg == BYTE_LAST && i_tx_done)
        begin
          if (regfile_addr_reg == {NB_REG_ADDR{1'b0}})
          begin
            next_state = IDLE                                                                     ;
          end
          else
