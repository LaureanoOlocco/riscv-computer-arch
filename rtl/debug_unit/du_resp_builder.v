...
#(
    ...
    // Latched response fields
    reg [NB_UART_DATA - 1 : 0] status_reg, status_next;
    reg [NB_DATA - 1 : 0]      data_reg, data_next;

    // Byte counter (0=status, 1-4=data bytes)
    reg [NB_COUNTER - 1 : 0] byte_cnt_reg, byte_cnt_next;
    ...
    always @(*) begin
        ...
        case (state_reg)
            ...
            WAIT_TX: begin
                if (i_tx_done && byte_cnt_reg == 3'd5) begin
                    o_done = 1'b1;
                end
            end
